...
func Initialize() *F2SQueue {
	f2squeue := F2SQueue{
		Requests:     make([]F2SInvocationRequest, 0),
		eventChannel: make(chan *F2SInvocationRequest),
	}
...
func (queue *F2SQueue) AddRequest(req *F2SInvocationRequest) {
	logging.Info("adding request to queue")
	queue.eventChannel <- req
}

// add a new request to the queue
func (queue *F2SQueue) RequestDone(req F2SInvocationRequest) {
	logging.Info(fmt.Sprintf("request %s has completed", req.Metadata.Uid))
	for x, request := range queue.Requests {
		if request.Metadata.Uid == req.Metadata.Uid {
